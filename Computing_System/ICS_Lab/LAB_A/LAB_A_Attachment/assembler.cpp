...
std::string ConvertBin2Hex(std::string bin)  {
    ...
    temp << std::hex << std::stoi(bin, nullptr, 2);
    temp >> answer;
    //std::transform(answer.begin(), answer.end(), answer.begin(), ::toupper);    //转换为大写
    return answer;
}
...
std::string assembler::TranslateOprand(int current_address, std::string str, int opcode_length) {
    ...
    if (!(item.getType() == vAddress && item.getVal() == -1)) {
        ...
        if (item.getType() == vAddress) {
            auto offset = item.getVal() - current_address - 1;
            std::string midanswer = NumberToAssemble(offset);
            std::string answer = "";
            answer.append(midanswer, 16 - opcode_length, opcode_length);
            return answer;
        }
    ...
    if (str[0] == 'R') {
        ...
        // TO BE DONE.
        auto temp = RecognizeNumberValue(str);
        std::string midanswer = NumberToAssemble(temp);
        std::string answer = "";
        answer.append(midanswer, 16 - opcode_length, opcode_length);
        return answer;
...
int assembler::assemble(std::string input_filename, std::string output_filename) {
    ...
    for (int line_index = 0; line_index < file_content.size(); ++line_index) {
        ...
        if (file_tag[line_index] == lOperation) {
            ...
            // TO BE DONE.
            auto parameter_str_index = 0;
            const std::string comma = ",";
            const std::string whitespace = " ";
            while (parameter_str_index < parameter_str.size()) {
                auto location = parameter_str.find(comma, parameter_str_index);
                if (location == -1)
                    break;
                parameter_str.replace(location, 1, whitespace);
                parameter_str_index = location + 1;
            }
